...
- 2026-06-20 — **לוקליזציה מלאה של הממשק האנגלי + ניקוי deprecations.** (1) **בייליין:** עברית הופרדה ל-"ערן אבן־חיים · פילוסופיה · מתמטיקה · מדע · תודעה" (גם `subtitle` וגם `masthead.subtitle`); נוסף override אנגלי `languages.en.params.subtitle`+`masthead.subtitle`+`masthead.title:"Illuminism"` (קודם ה-header הציג עברית ב-`/en/`). (2) **author אנגלי:** `languages.en.params.author: "Eran Even-Haim"` (חל על כל הפוסטים האנגליים). (3) **תפריט:** `languages.en.menu.main` → "Tags" (`/en/tags/`); העברי נשאר "תגיות". (4) **i18n:** נוצרו `i18n/en.yaml` + `i18n/he.yaml` (override/השלמה ל-PaperMod) עם מפתחות: pinTag, ctaReadInOrder, navToggle, ariaPostLink/Quotes/SeqNav, seqNext/seqPrev, err404*. כל המחרוזות ה-hardcoded ב-`layouts/index.html`, `_partials/post_nav_links.html`, `404.html` הוחלפו ב-`{{ i18n }}`. תוצאה: תג "Pinned · Start here", CTA "Start reading in order →", ניווט "Next/Previous in series", 404 — כולם אנגלית ב-`/en/`, עברית ב-`/`. (5) **deprecations:** `languageName→label`, `languageCode→locale`, `languagedirection→direction` (he+en); `.Language.LanguageName→.Language.Label` ב-`_partials/header.html` ובאוברייד חדש `_partials/translation_list.html`. **build נקי לחלוטין — 0 אזהרות** (HE 69 / EN 38). (6) **תיקון em-dash** ב-`content/posts/000-welcome.md` (summary) → `–`. (7) **7 ציטוטים אנגליים** הוחלפו בנוסח קנוני שמסר ערן (ראסל, ארנו 1686, דייויס, וילצ'ק, צילינגר, סוסקינד, ניטשה #1). **חריג מכוון:** סרגל האדמין וקישורי "✏️ ערוך" נשארו עברית — כלי עורך בלבד, מוסתרים מ-visitor רגיל (CSS, `?admin=1`). יחיד עברי גלוי שנותר ב-`/en/`: תווית מתג השפה "עברית" (תקין).
